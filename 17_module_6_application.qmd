...
# M6A: Wrangling and Tidying Data {#sec-wrangling}
...
### Needed packages {#sec-wrangling-packages .unnumbered}

Let's load all the packages needed for our wrangling practice (this assumes you've already installed them). If needed, read Section @sec-packages for information on how to install and load R packages.
...
1. The starting value `x` will be a data frame. For example, the \index{R packages!nycflights23} `flights` data frame we explored in Section @sec-nycflights.
...
Keep in mind, there are many more advanced data-wrangling functions than just the six listed in the introduction to this chapter; you'll see some examples of these in Section @sec-other-verbs. However, just with these six verb-named functions you'll be able to perform a broad array of data-wrangling tasks for the rest of this book.
...
As we progressively include more airports, this will get unwieldy to write. A slightly shorter approach uses the `%in%` \index{operators!in} operator along with the `c()` function. Recall from Subsection @sec-programming-concepts that the `c()` function "combines" or "concatenates" values into a single *vector* of values. \index{vectors}
...
Let's take a look at only the `dep_delay`, `arr_delay`, and the resulting `gain` variables for the first 5 rows in our updated `flights` data frame in Table @tbl-first-five-flights.
...
The values in the variable `carrier` in the `flights` data frame match the values in the variable `carrier` in the `airlines` data frame. In this case, we can use the variable `carrier` as a \index{joining data!key variable} *key variable* to match the rows of the two data frames. Key variables are almost always *identification variables* that uniquely identify the observational units as we saw in Subsection @sec-identification-vs-measurement-variables. This ensures that rows in both data frames are appropriately matched during the join. Hadley and Garrett [@rds2016] created the diagram in Figure @fig-reldiagram to show how the different data frames in the `nycflights23` package are linked by various key variables:
...
A visual representation of the `inner_join()` is shown in Figure @fig-ijdiagram [@rds2016]. There are other types of joins available (such as `left_join()`, `right_join()`, `outer_join()`, and `anti_join()`), but the `inner_join()` will solve nearly all of the problems you'll encounter in this book.
...
However, if you look at both the `airports` and `flights` data frames, you'll find that the airport codes are in variables that have different names. In `airports`, the airport code is in `faa`, whereas in `flights` the airport codes are in `origin` and `dest`. This fact is further highlighted in the visual representation of the relationships between these data frames in Figure @fig-reldiagram.
...
Say instead we want to join two data frames by *multiple key variables*. For example, in Figure @fig-reldiagram, we see that in order to join the `flights` and `weather` data frames, we need more than one key variable: `year`, `month`, `day`, `hour`, and `origin`. This is because the combination of these 5 variables act to uniquely identify each observational unit in the `weather` data frame: hourly weather recordings at each of the 3 NYC airports.

We achieve this by specifying a *vector* of key variables to join by using the `c()` function. Recall from Subsection @sec-programming-concepts that `c()` is short for "combine" or "concatenate." \index{vectors}
...
In Subsection @sec-programming-concepts, I introduced the concept of a \index{data frame} data frame in R: a rectangular spreadsheet-like representation of data where the rows correspond to observations and the columns correspond to variables describing each observation.  In Section @sec-nycflights, we started exploring our first data frame: the `flights` data frame included in the `nycflights23` package. 
...
### Needed packages {#sec-tidy-packages .unnumbered}

Let's load all the packages needed for this walkthrough (this assumes you've already installed them). If needed, read Section @sec-packages for information on how to install and load R packages.
...
Up to this point, most of our activities have used data stored inside of an R package, though we *have* practiced importing data from an outside source in Section @sec-loading-dataset. It's time to put that practice to use!
...
Let's apply some of the data wrangling verbs we learned in Section @sec-wrangling on the `drinks` data frame:
...
What does it mean for your data to be "tidy"? While "tidy" has a clear English meaning of "organized," the word "tidy" in data science using R means that your data follows a standardized format. We will follow Hadley Wickham's (2014) definition of *"tidy" data*  shown also in Figure @fig-tidyfig:
...
```{r fig-tidyfig, echo=FALSE, fig.cap="Tidy data graphic from *R for Data Science*.", out.height="80%", out.width="80%", purl=FALSE}
knitr::include_graphics("images/r4ds/tidy-1.png")
```

For example, say you have the following table of stock prices in Table @tbl-non-tidy-stocks:
...
Although the data are neatly organized in a rectangular spreadsheet-type format, they do not follow the definition of data in "tidy" format. While there are three variables corresponding to three unique pieces of information (date, stock name, and stock price), there are not three columns. In "tidy" data format, each variable should be its own column, as shown in Table @tbl-tidy-stocks. Notice that both tables present the same information, but in different formats. 

Although the data is in a rectangular spreadsheet format, it is not "tidy." There are three variables (date, stock name, and stock price), but not three separate columns. In tidy data, each variable should have its own column, as shown in Table @tbl-tidy-stocks. Both tables present the same information, but in different formats.
...
On the other hand, consider the data in Table @tbl-tidy-stocks-2.
...
In this case, even though the variable "Boeing Price" occurs just like in our non-"tidy" data in Table @tbl-non-tidy-stocks, the data *is* "tidy" since there are three variables for each of three unique pieces of information: Date, Boeing price, and the Weather that day.
...
Let's use the `dem_score` data frame we imported in Section @sec-csv, but focus on only data corresponding to Guatemala.
...
Now we are stuck in a predicament, much like with our `drinks_smaller` example in Section @sec-tidy-data-ex. We see that we have a variable named `country`, but its only value is `"Guatemala"`.  We have other variables denoted by different year values.  Unfortunately, the `guat_dem` data frame is not "tidy."
...
After installing the `tidyverse` package as you would a normal package (as seen in Section @sec-packages), running:
