...
If you are already somewhat familiar with these concepts, feel free to skip to Section @sec-nycflights where we'll introduce our first dataset: all domestic flights departing one of the three main New York City (NYC) airports in 2023. We will explore this dataset in depth for much of the rest of this book.



## What are R and RStudio? {#sec-r-rstudio}

Throughout this book, I will expect that you are using R via RStudio. First time users often confuse the two. At its simplest, R is like a car's engine\index{R} while RStudio is like a car's dashboard\index{RStudio} as illustrated in Figure @fig-R-vs-RStudio-1.
...
Much as we don't drive a car by interacting directly with the engine but rather by interacting with elements on the car's dashboard, we won't be using R directly but rather we will use RStudio's interface. After you install R and RStudio on your computer, you'll have two new *programs* (also called *applications*) you can open. We'll always work in RStudio and not in the R application. Figure @fig-R-vs-RStudio-2 shows what icon you should be clicking on your computer. 
...
After you open RStudio, you should see something similar to Figure @fig-RStudio-interface. (Note that this interface changes over time and that it's possible to customize it, so don't be surprised if what you see on your—or a classmate's—screen is slightly different.)
...
Completing actual projects in R can be more helpful for learning the software than reading written descriptions alone. Rather than asking you to memorize terms and concepts, this course uses walkthroughs (like this one) so that you can “learn by doing.” To help with these walkthroughs, a different font distinguishes regular text from `computer_code`.
...
* *Data frames*: rectangular spreadsheets. They are representations of datasets in R where the rows correspond to *observations* and the columns correspond to *variables* that describe the observations. \index{data frame} We'll cover data frames later in Section @sec-nycflights.
...
For example, among the many packages we will use in this book are the `ggplot2` package [@R-ggplot2] for data visualization in Chapter @sec-viz\index{R packages!ggplot2}, the `dplyr` package [@R-dplyr] for data wrangling in Chapter @sec-wrangling\index{R packages!dplyr}, the `moderndive` package [@R-moderndive] that accompanies this book\index{R packages!moderndive}, and the `infer` package [@R-infer] for "tidy" and transparent statistical inference in Chapters @sec-confidence-intervals, @sec-hypothesis-testing, and @sec-inference-for-regression\index{R packages!infer}.

A good analogy for R packages \index{R packages} is they are like apps you can download onto a mobile phone like in Figure @fig-R-vs-R-packages:
...
There are two ways to install an R package: an easy way and a more advanced way. This course uses the more advanced method because class projects require you to use it, so it is helpful to establish that habit now.
...
`...` it means that you didn't successfully install it. This is an example of an "error message" we discussed in Subsection @sec-messages. If you get this error message, go back to Subsection @sec-package-installation on R package installation and make sure to install the `ggplot2` package before proceeding. 
...
Let's first load all the packages needed for this chapter, assuming you've already installed them. Read Section @sec-packages for information on how to install and load R packages if you haven't already.
...
If you are new to the world of coding, R, and RStudio and feel you could benefit from a more detailed introduction, we suggest the short book, [*Getting Used to R, RStudio, and R Markdown*](https://rbasics.netlify.app/) [@usedtor2016], previewed in Figure @fig-ismay-book. It includes screencasts that you can follow along and pause as you learn. This book also contains an introduction to R Markdown, a tool used for reproducible research.


```{r fig-ismay-book, echo=FALSE, fig.cap="Preview of *Getting Used to R, RStudio, and R Markdown*.", purl=FALSE, out.width=if(knitr::is_latex_output()) "82%"}
